...
public class Logger {
    ...
    public Logger(String logFilePath) {
        ...
        this.logFilePath = new File(rootDir.toFile(), logFilePath).getAbsolutePath();
        // this.logFilePath = new File(System.getProperty("user.dir"),
    ...
    private void initializeLogFile() {
        ...
        if (!logFile.exists() || logFile.length() == 0) {
            try (PrintWriter pw = new PrintWriter(new FileWriter(logFile, true))) {
                pw.println(
                        "Timestamp,Level,Component,Operation,Message,Duration(ms),CPU_Usage(%),Memory_Usage(MB),FileSize,ThreadID");
            } catch (IOException e) {
    ...
    public synchronized void logEvent(
        ...
        // CSV logging
        double cpuUsage = 0; // getCPUUsage();
        double memMB = getMemoryUsage() / (1024.0 * 1024.0);
        long threadId = Thread.currentThread().getThreadGroup().activeCount();
        // long threadId = Thread.currentThread().getId(); // TODO: to be implemented in
        // the last column
